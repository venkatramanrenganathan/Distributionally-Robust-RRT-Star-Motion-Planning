function status = DRCheckingCollision(path_check_param)
...
for i = 1:size(Obstacles,1) % check if sample hits an obstacle
    ...
    x_tight   = sqrt((1-alpha(i))/alpha(i))*norm(cov_matrix*a_x) + add_p;
    y_tight   = sqrt((1-alpha(i))/alpha(i))*norm(cov_matrix*a_y) + add_p;
    x_v_tight = sqrt((1-alpha(i))/alpha(i))*norm(cov_matrix*x_v) + add_p;
    y_v_tight = sqrt((1-alpha(i))/alpha(i))*norm(cov_matrix*y_v) + add_p;
    
    if x(1) >= Obstacles(i,1) - x_tight && ...
       x(1) <= Obstacles(i,1) + Obstacles(i,3) + x_v_tight && ...
       x(2) >= Obstacles(i,2) - y_tight && ...
       x(2) <= Obstacles(i,2) + Obstacles(i,4) + y_v_tight
        hits = hits + 1;        
    end    
