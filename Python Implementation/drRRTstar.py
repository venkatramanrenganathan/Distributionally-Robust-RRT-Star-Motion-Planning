...
class DR_RRTStar():
    ...
    def __init__(self, start, randArea, maxIter):
        ...
        self.alfa           = [0.1, 0.1, 0.1, 0.1] # [0.09 + (0.13-0.09)*random.random() for i in range(len(self.obstacleList))]                 
    ...
    def DRCollisionCheck(self, trajNode):
        ...
        for alfa, (ox, oy, wd, ht) in zip(self.alfa, self.obstacleList):                        
            ...
            if (trajNode.X[0] >= ox + math.sqrt((1-alfa)/alfa)* LA.norm(np.dot(trajNode.Sigma, np.array([1,0,0,0]))) and      # Left 
                trajNode.X[0] <= ox + wd - math.sqrt((1-alfa)/alfa)*LA.norm(np.dot(trajNode.Sigma, np.array([1,0,0,0]))) and  # Right
                trajNode.X[1] >= oy + math.sqrt((1-alfa)/alfa)*LA.norm(np.dot(trajNode.Sigma, np.array([0,1,0,0])))  and      # Bottom
                trajNode.X[1] <= oy + ht - math.sqrt((1-alfa)/alfa)*LA.norm(np.dot(trajNode.Sigma, np.array([0,1,0,0]))) ):   # Top
    ...
    def LineRectangleCollisionFreeCheck(self, fromPoint, toPoint):
        ...
        for alfa, (ox, oy, wd, ht) in zip(self.alfa, self.obstacleList): 
            ...
            minX = ox - math.sqrt((1-alfa)/alfa)*LA.norm(np.dot(toPoint.Sigma, np.array([1,0,0,0])))
            minY = oy - math.sqrt((1-alfa)/alfa)*LA.norm(np.dot(toPoint.Sigma, np.array([0,1,0,0])))
            maxX = ox + wd + math.sqrt((1-alfa)/alfa)*LA.norm(np.dot(toPoint.Sigma, np.array([1,0,0,0])))        
            maxY = oy + ht + math.sqrt((1-alfa)/alfa)*LA.norm(np.dot(toPoint.Sigma, np.array([0,1,0,0])))
