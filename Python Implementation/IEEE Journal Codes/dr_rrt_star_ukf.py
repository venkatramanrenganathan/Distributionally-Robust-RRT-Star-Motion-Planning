...
and state estimation performed using Unscented Kalman Filter. 
...
class trajNode():
    ...
    def __init__(self):
        ...
        """
        self.X     = np.zeros((3, 1)) # State Vector [x-position, y-position, theta-orientation]
        self.Sigma = np.zeros((3, 3)) # Covariance Marix
...
class DR_RRTStar_Node():
    ...
    def __init__(self):
        ...
        self.means  = np.zeros((STEER_TIME+1, 3, 1)) # Mean Sequence
        self.covar  = np.zeros((STEER_TIME+1, 3, 3)) # Covariance Sequence        
...
class DR_RRTStar():
    ...
    def GetDynamicsData(self):
        ...
        S0 = np.array([[0.01, 0,    0], 
                         [0,    0.02, 0], 
                         [0,    0,    0.001]])                            # Initial State Covariance                           
    ...
    def DRSTLCollisionCheck(self, firstNode, secondNode):
        ...
        for alfa, (ox, oy, wd, ht) in zip(self.alfa, self.obstacleList):                        
            ...
            invCDF = erfinv(1-2*alfa)
            # left,right,bottom,top
            # Check if the firstNode is inside the bloated obstacle
            if (firstNode.X[0] <= ox - math.sqrt(2*np.dot(xDir, np.dot(firstNode.Sigma, xDir)))*invCDF and
                firstNode.X[0] >= ox + wd + math.sqrt(2*np.dot(xDir,np.dot(firstNode.Sigma, xDir)))*invCDF and
                firstNode.X[1] >= oy + math.sqrt(2*np.dot(yDir, np.dot(firstNode.Sigma, yDir)))*invCDF and
                firstNode.X[1] <= oy + ht - math.sqrt(2*np.dot(yDir,np.dot(firstNode.Sigma, yDir)))*invCDF ):   # Top
                return False    # collision has occured
            if (secondNode.X[0] <= ox - math.sqrt(2*np.dot(xDir, np.dot(secondNode.Sigma, xDir)))*invCDF and
                secondNode.X[0] >= ox + wd + math.sqrt(2*np.dot(xDir,np.dot(secondNode.Sigma, xDir)))*invCDF and
                secondNode.X[1] >= oy + math.sqrt(2*np.dot(yDir,np.dot(secondNode.Sigma, yDir)))*invCDF and
                secondNode.X[1] <= oy + ht - math.sqrt(2*np.dot(yDir,np.dot(secondNode.Sigma, yDir)))*invCDF ):   # Top
                return False    # collision has occured
            
            # Also check if both waypoints are on same side of obstacles
            # Return false if both waypoints are not on same side of obstacles            
            leftCdtn   = (firstNode.X[0] <= ox - math.sqrt(2*np.dot(xDir, np.dot(firstNode.Sigma, xDir)))*invCDF and
                          secondNode.X[0] <= ox - math.sqrt(2*np.dot(xDir, np.dot(secondNode.Sigma, xDir)))*invCDF)
            rightCdtn  = (firstNode.X[0] >= ox + wd + math.sqrt(2*np.dot(xDir,np.dot(firstNode.Sigma, xDir)))*invCDF and
                          secondNode.X[0] >= ox + wd + math.sqrt(2*np.dot(xDir,np.dot(secondNode.Sigma, xDir)))*invCDF) 
            bottomCdtn = (firstNode.X[1] >= oy + math.sqrt(2*np.dot(yDir,np.dot(firstNode.Sigma, yDir)))*invCDF and
                          secondNode.X[1] >= oy + math.sqrt(2*np.dot(yDir,np.dot(secondNode.Sigma, yDir)))*invCDF)
            topCdtn    = (firstNode.X[1] <= oy + ht - math.sqrt(2*np.dot(yDir,np.dot(firstNode.Sigma, yDir)))*invCDF and
                          secondNode.X[1] <= oy + ht - math.sqrt(2*np.dot(yDir,np.dot(secondNode.Sigma, yDir)))*invCDF)
            
            # Return False if either of the above 4 cdtns are satisfied
            if leftCdtn or rightCdtn or bottomCdtn or topCdtn:
                return False    # Collision will occur btw waypoints & obstacle 
    ...
    def LineRectangleCollisionFreeCheck(self, fromPoint, toPoint):
        ...
        for alfa, (ox, oy, wd, ht) in zip(self.alfa, self.obstacleList): 
            ...
            minX = ox - math.sqrt(2*np.dot(np.array([1,0,0,0]),np.dot(toPoint.Sigma, np.array([1,0,0,0]))))*invCDF
            minY = oy - math.sqrt(2*np.dot(np.array([0,1,0,0]),np.dot(toPoint.Sigma, np.array([0,1,0,0]))))*invCDF
            maxX = ox + wd + math.sqrt(2*np.dot(np.array([1,0,0,0]),np.dot(toPoint.Sigma, np.array([1,0,0,0]))))*invCDF
            maxY = oy + ht + math.sqrt(2*np.dot(np.array([0,1,0,0]),np.dot(toPoint.Sigma, np.array([0,1,0,0]))))*invCDF
    ...
    def ConnectViaMinimumCostPath(self, nearestIndex, nearIndices, randNode, minNode):
        ...
        for j, nearIndex in enumerate(nearIndices):            
            ...
            xTrajs, sequenceCost = nmpc(self.nodeList[nearIndex], 
                                        randNode)             
            ...
            for k, xTraj in enumerate(xTrajs):    
                ...
                if k != 0:
                    drSTLCollisionFreeFlag = self.DRSTLCollisionCheck(xTrajs[k], 
                                                                      xTrajs[k-1])
    ...
    def ReWire(self, nearIndices, minNode):
        ...
        for j, nearIndex in enumerate(nearIndices):                                  
            ...
            xTrajs, sequenceCost = nmpc(minNode, 
                                        self.nodeList[nearIndex]) 
    ...
    def UpdateDescendantsCost(self, newNode):
        ...
        for childNode in self.nodeList[newNodeIndex:]:            
            ...
            if childNode.parent == newNodeIndex:  
                ...
                xTrajs, trajCost = nmpc(self.nodeList[newNodeIndex], 
                                        childNode)
    ...
    def GetGoalNodeIndex(self):        
        ...
        goalIndices = []
        for node in self.nodeList:            
            if (node.means[-1,0,:] >= self.xmingoal and 
                node.means[-1,1,:] >= self.ymingoal and 
                node.means[-1,0,:] <= self.xmaxgoal and 
                node.means[-1,0,:] <= self.ymaxgoal):
    ...
    def PlotObstacles(self):
        ...
        plt.plot(self.start.means[-1,0,:], self.start.means[-1,1,:], 
                 "Xr", markersize = 20)        
    ...
    def DrawGraph(self, lastFlag, randNode=None):                
        ...
        rx, = plt.plot(randNode.means[-1,0,:], 
                       randNode.means[-1,1,:], "^k") 
    ...
    def ExpandTree(self):        
        ...
        for iter in range(self.maxIter): 
            ...
            xTrajs, trajCost = nmpc(self.nodeList[nearestIndex], randNode) 
        ...
        # Select a goal node from the tree that lies in the goal area
        goalNodeIndex = self.GetGoalNodeIndex()
        # Generate the sampled path from goal node to root node
        pathNodesList = self.GenerateSamplePath(goalNodeIndex)
...
def main():    
    ...
    if pathNodesList is not None:
        ...
        for pathNode in pathNodesList: 
            ...
            elE, elV = LA.eig(elcovar[0:2,0:2])
            alfa     = math.atan2(pathNode.means[-1,1,0], pathNode.means[-1,0,0])
            angleValues.append(alfa*360)                  
